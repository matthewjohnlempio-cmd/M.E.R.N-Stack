...
export default function Dashboard() {
  ...
  // Fetch products from Firebase for the current user
  useEffect(() => {
    if (!currentUser) {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const dbRef = ref(realtimeDb, `userProducts/${currentUser.uid}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const userProductsRef = ref(realtimeDb, `userProducts/${currentUser.uid}`);
      if (editingId) {
        // Update product in Firebase under user's node
        await update(ref(realtimeDb, `userProducts/${currentUser.uid}/${editingId}`), form);
        setProducts(
          products.map((p) => (p.id === editingId ? { id: editingId, ...form } : p))
        );
      } else {
        // Add new product under user's node
        const newRef = ref(realtimeDb, `userProducts/${currentUser.uid}/${Date.now()}`);
        await set(newRef, form);
        setProducts([{ id: newRef.key, ...form }, ...products]);
      }
      resetForm();
    } catch {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await remove(ref(realtimeDb, `userProducts/${currentUser.uid}/${id}`));
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-6 py-6">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-400">Welcome back, {currentUser?.email}</p>
          </div>
